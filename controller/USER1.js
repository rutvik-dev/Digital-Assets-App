...
let SignupUser = async function (req, res, next) {
    try {
        const { username, email, password , AssetId} = req.body;

        const hashedPassword = await bcrypt.hash(password, 10);

        const CreateUser = await User.create({ username, email, password: hashedPassword , AssetId});
...
let ReadAllUser = async function (req, res, next) {
    try {
        ...
        const ReadUsers = await User.find().populate('AssetId');
        res.status(201).json({
            status: "success",
...
let ReadSingleUser = async function (req, res, next) {
    try {
        ...
        const user = await User.findById(id).populate('AssetId');
        if (!user) {
...
let UpdateUser = async function (req, res, next) {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(id, req.body, { new: true }).populate('AssetId');
        if (!updatedUser) {
            return res.status(404).json({ message: 'User not found' });
        }
        res.status(201).json({
